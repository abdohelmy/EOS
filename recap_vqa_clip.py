from recaption_model_vqa_T5 import blip_recaption, bert_recaption
from recap_dataset_T5 import Coco_Dataset, Coco_Dataset_things, Coco_Dataset_attributes, Coco_Dataset_Ref, DOCCI, Coco_Dataset_VQA, Coco_Dataset_dummy, FineCapEval, DCI
...
def val_evaluation(model, dataloader_val, ref_caps):
    ...
    with tqdm( unit='it', total=len(dataloader_val)) as pbar:
        ...
        for it, (_,_, captions,_,images, prompts, ids,_) in enumerate(dataloader_val):
            images, prompts = images.to(device), prompts.to(device)
            ...
            with torch.no_grad():
                ...
                # generated_ids = model.model_blip.generate(input_ids=prompts, pixel_values=images, max_length=60,num_beams= 5)
                generated_ids_contrastive  = model.model_blip.generate(input_ids=prompts, pixel_values=images, max_length=120, output_logits= True, return_dict_in_generate=True) #greedy
                generated_ids_beam = model.model_blip.generate(input_ids=prompts, pixel_values=images, max_length=100,num_beams= 5, no_repeat_ngram_size=2, repetition_penalty=1.5)
                logits = generated_ids_contrastive[1]

                logits = torch.stack(logits, dim=1)
                indices = generated_ids_contrastive[0].unsqueeze(-1)[:,1:,:]
                ...
                # generated_ids = model.model.generate(input_ids=prompts, pixel_values=images, max_length=60)
                generated_caption = processor.batch_decode(generated_ids_contrastive[0], skip_special_tokens=True)
                output_contrastive = {key: [value] for key,value in zip(ids[0], generated_caption)}
                generated_caption = processor.batch_decode(generated_ids_beam, skip_special_tokens=True)
                output_beam = {key: [value] for key,value in zip(ids[0], generated_caption)}
                generated_ids_beam_l = model.model_blip.generate(input_ids=prompts, pixel_values=images, max_length=100,num_beams= 5, no_repeat_ngram_size=3, repetition_penalty=1.2)

                generated_caption = processor.batch_decode(generated_ids_beam_l, skip_special_tokens=True)
                output_beam_l = {key: [value] for key,value in zip(ids[0], generated_caption)}
            ...
            gen_caps_contrastive = {**gen_caps_contrastive, **output_beam_l}
    ...
    CHAIR_data_beam = [{"image_id": int(key), "caption": caption} for key, captions in gen_caps_beam.items() for caption in captions]
    CHAIR_data_beam_l = [{"image_id": int(key), "caption": caption} for key, captions in gen_caps_contrastive.items() for caption in captions]
    print(gen_caps_beam)
    ...
    return gen_caps_contrastive, gen_caps_beam, np.mean(all_probs), np.mean(all_probs_rest), avg_len/it, recall_c.mean(), distinct_ns/it, CHAIR_data_beam, CHAIR_data_beam_l
...
transform = T.Compose([
    ...
    T.Normalize(                    # Normalize using CLIP defaults
...
def recaptioning(things_coco_dataloader_train, coco_dataloader_val, coco_cider_dataloader, ref_caps, model_captioning, model_downstream, optimizer1, optimizer2, weights):
    ...
    loss_fn = torch.nn.CrossEntropyLoss(reduction='none')
    # model_captioning_state_dict = torch.load('/workspace/T5_pod/T5_checkpoint_clip.pth', map_location='cpu')
    ...
    # ref_caps = load_references_from_json("./my_datasets/captions_val2014.json")
    logging_step = 0
    clip_score_fn = CLIPScore(model_name_or_path="openai/clip-vit-large-patch14-336").to("cuda")
    ref_caps = load_references_from_json_val("./my_datasets/captions_val2014.json")
    running_length = 0.0
    running_avg = 0.0
    lowest_prob = 10
    ...
    with wandb.init(project="recap", mode=WANDB_STATUS, notes=NOTES, name=EXP_NAME, config= configs):
        for ep in range(0,50):
            ...
            attn_mask_fine = []
            labels_gold = []
            # c_contrastive, c_beam, r_contrastive, r_beam, recall_contrastive, dict_base = evaluation(model_captioning, coco_dataloader_val, ref_caps)
            ...
            # with open("dict_delay_eos_base.jsonl", "w") as f:
...
#                               "/fsx/homes/Abdelrahman.Mohamed@mbzuai.ac.ae/abdo/recap_T5/recaptioning_T5_norepeat_last3_eos_025.pth",
...
#                               "/l/users/abdelrahman.mohamed/T5/recaptioning_T5_last_eos_025.pth",
...
#                     with open("dict_0_25_T5_F_contrastive.jsonl", "w") as f:
#                         for item in dict_base:
#                             f.write(json.dumps(item) + "\n")
#                     with open("dict_0_25_T5_F_beam.jsonl", "w") as f:
...
#             quit()
            ...
            with tqdm( unit='it', total=len(things_coco_dataloader_train)) as pbar:
                try:
                    ...
                    for  it,(label, answer, captions, ques, images, prompts, image_id, clip_img) in  enumerate(things_coco_dataloader_train):
                        # if it%250==0:
                        ...
                        if it%75==0:
                            ...
                            captions_greedy, captions_beam, probs_mean, rest_probs_mean, avg_length, recalls, n_grams, chairs, chairs_l= val_evaluation(model_captioning, coco_dataloader_val, ref_caps)
                            ...
                            # with open("chair.jsonl", "w") as f:
                            #     for item in chairs:
                            #         f.write(json.dumps(item) + "\n")
                            # valuator = pickle.load(open("chair.pkl", 'rb'))
                            # chair_dic = valuator.compute_chair("/workspace/T5_pod/chair.jsonl","image_id","caption")
                            # chair_i = chair_dic["overall_metrics"]['CHAIRi']*100
                            # chair_s = chair_dic["overall_metrics"]['CHAIRs']*100

                            # recall_i = chair_dic["overall_metrics"]['Recall']
                            # wandb.log({"Recall_ch":recall_i})
                            # wandb.log({"CHAIR_i":chair_i})
                            # wandb.log({"CHAIR_s":chair_s})
                            # recalls_lst.append(recalls)
                            # chair_lst.append(chair_i)
                            # if it>0:
                            #     grad_chair = np.gradient(np.array(chair_lst))
                            #     grad_recall = np.gradient(np.array(recalls_lst))
                            #     wandb.log({"grad_chair_m":grad_chair.mean()})
                            #     wandb.log({"grad_recall_m":grad_recall.mean()})
                            #     wandb.log({"grad_recall_l":grad_recall[-1]})
                            #     wandb.log({"grad_recall_l":grad_recall[-1]})
                            # with open("chair.jsonl", "w") as f:
                            #     for item in chairs_l:
                            #         f.write(json.dumps(item) + "\n")
                            # chair_dic = valuator.compute_chair("/workspace/T5_pod/chair.jsonl","image_id","caption")
                            # chair_i = chair_dic["overall_metrics"]['CHAIRi']*100
                            # chair_s = chair_dic["overall_metrics"]['CHAIRs']*100
                            ...
                            # recall_i = chair_dic["overall_metrics"]['Recall']
                            # wandb.log({"Recall_ch_l":recall_i})
                            # wandb.log({"CHAIR_i_l":chair_i})
                            # wandb.log({"CHAIR_s_l":chair_s})
                            # wandb.log({"Recall_ch_l":recall_i})
                            ...
                            if lowest_prob>rest_probs_mean-0.01:
                                lowest_prob = rest_probs_mean
                                patience=0
                            else:
                                patience+=1
                            if it == 301:
                                torch.save(model_captioning.state_dict(), './T5_checkpoint_clip.pth')
                                # quit()
                            ...
                            # if probs_mean<0.3 and probs_mean >0.2:
                            # if probs_mean<0.2 and probs_mean>0.15:
                            #     torch.save(model_captioning.state_dict(), './recaptioning_T5_con_last4_eos_015.pth')
                            # if probs_mean<0.1:
                            #     torch.save(model_captioning.state_dict(), './recaptioning_T5_con_last4_eos_01.pth')
                    # with torch.autograd.set_detect_anomaly(True):
                        ...
                        # caption_dict = {key: [value] for key,value in zip(image_id[0], gt_caption)}
                            ...
                            rouge, recall_recap = evaluate_rouge(generated_caption_baseline_dict, caption_dict) #use ref caps since we are using dict
                        ...
                        # try:
                        # if it<50 or it>120:
                        # outs  = model_captioning.model_blip.generate(input_ids=prompts, pixel_values=images,penalty_alpha=0.6, top_k=3,  max_length=60, output_logits= True, return_dict_in_generate=True  )
                        outs  = model_captioning.model_blip.generate(input_ids=prompts, pixel_values=images, max_length=50,num_beams= 5, output_logits= True, return_dict_in_generate=True )
                        ...
                        # outs  = model_captioning.model_blip.generate(input_ids=prompts, pixel_values=images, do_sample=True, top_p=0.92, top_k=0 , max_length=35,output_logits= True, return_dict_in_generate=True  )https://wandb.ai/abdohelmy/recap/workspace
                        ...
                        # else:
                        # outs  = model_captioning.model_blip.generate(input_ids=prompts, pixel_values=images, max_length=60,output_logits= True, return_dict_in_generate=True )
                        ...
                        #     flag = 1
                        #     continue
                        generated_tokens_sampling = outs[0].cpu().detach()[:,1:]
                        logits = outs[1]
                        #what if i did not detach?
                        # rewards_img, rewards_context = outs[1]
                        # rewards_img = rewards_img.detach()
                        # rewards = outs[1]
                        logits = torch.stack(logits, dim=1)
                        indices = generated_tokens_sampling.unsqueeze(-1).to("cuda:0")
...
######################## using log_probs############################################
                        ...
                        beam_score_c = evaluate_cider(generated_caption_random_dict, caption_dict)
                        # reward_cidar = recap_score_c - base_score_c
                        # generated_caption_random_dict_base = {key: [value] for key,value in zip(image_id[0], generated_caption)}
                        eos_all = torch.exp(logits_prob[:,:,1]).detach().cpu()
                        eos_values = []
                        top2_values = []
                        top2_indices = []
                        for probs,inds in zip(eos_all, eos_indices):
                            if len (top2_indices) ==0:
                                top2_values, top2_indices = torch.topk(probs[0:inds[1]+1], 2, dim=0)
                            else:
                                top2_value, top2_indice = torch.topk(probs[0:inds[1]+1], 2, dim=0)

                                top2_values = torch.vstack((top2_values,top2_value))
                                top2_indices = torch.vstack((top2_indices,top2_indice))
                        try:
                            EOS_dict = {key: [(value1.item(),np.round(value2.item() * 100) / 100,value3.item(),np.round(value4.item() * 100) / 100, value5.item(), np.round(value6.item() * 100) / 100)] for key,value1,value2,value3,value4, value5, value6 in zip(image_id[0], eos_indices[:,1], eos_probs,top2_indices[:,0], top2_values[:,0],top2_indices[:,1], top2_values[:,1])}
                        except:
                            pass
                        for v,i,e in zip (top2_values, top2_indices, eos_indices):
                            try:
                                # if e[1].item() != i[0].item():
                                #     print("$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$ early eos$$$$$$$$$$$$$$$$$$$$$$$")
                                    
                                wandb.log({"eos_propability": v[0].item()})
                                wandb.log({"eos_proportion": v[1].item()/v[0].item()})
                                wandb.log({"eos_pos_difference": i[0].item()-i[1].item()})
                            except:
                                pass
                        ...
                        with torch.no_grad():
                            # clip_img = clip_img.to("cuda")
                            # clip_img = torch.tensor(clip_img).to("cuda:0")
                            scores_base = []
                            scores_beam = []
                            # transform
                            image_tensors = [TF.resize(img, size=[224,224]).to(device) for img in clip_img]

                            # image_tensors = [transform(PIL.Image.fromarray(img)).to(device) for img in clip_img]

                            for image_tensor, caption in zip(image_tensors, generated_caption_baseline):
                                score = clip_score_fn(image_tensor, caption).detach()
                                scores_base.append(score.item())
                            for image_tensor, caption in zip(image_tensors, generated_caption):
                                score = clip_score_fn(image_tensor, caption).detach()

                                scores_beam.append(score.item())
                        #     

                        #     # batched_images = torch.stack(clip_img).to(device)
                        #     clip_score_greedy = clip_score_fn(image_tensors, generated_caption_baseline).detach()

                        #     clip_score_beam = clip_score_fn(image_tensors, generated_caption).detach()
                        # breakpoint()

                        reward_clip = torch.tensor(scores_beam) - torch.tensor(scores_base)
...
################################################################################
                        ...
                        #Mask newline Token
                        # try:
                        #     for vv in range (generated_tokens_sampling.shape[0]):
                        #         idx = np.where(generated_tokens_sampling[vv] == 50118)[0][0]
                        #         rewards[vv, 0:idx] = 0 
                        # except:
                        #     continue
                        # breakpoint()
                        reward_clip = reward_clip.to("cuda:0")
                        # breakpoint()
                        loss = -torch.mean(words_prob, -1) * reward_clip
                        ...
                        lang_proj_grad = torch.sqrt(torch.mean(model_captioning.model_blip.language_projection.weight.grad**2))
                        wandb.log({"language_proj_grad": lang_proj_grad})
                        ...
                        avg_seq_recap = np.mean(seq_recap)
                        running_length +=eos_probs.mean()
                        running_avg = running_length/it+1
...
if __name__ == "__main__":
    ...
    vqa_dataloader_train = DataLoader(vqa_train, batch_size=32,shuffle=True,collate_fn = vqa_train.collate_fn,
                                        drop_last=True) #with image
    vqa_dataloader_val = DataLoader(vqa_val, batch_size=10,shuffle=False, collate_fn = vqa_val.collate_fn,
                                        drop_last=True)


    # vqa_train_rec = FineCapEval(split="train", downstream = False)
    # vqa_val_rec = FineCapEval(split = "val", downstream= False)
    # vqa_val_rec = DCI(split = "val", downstream= False)
