...
class GenerateDecoderOnlyOutput(ModelOutput):
    ...
    similarities: Optional[Tuple[torch.FloatTensor]] = None
    ...
    def _contrastive_search(
        ...
        **model_kwargs,
        ...
        while self._has_unfinished_sequences(this_peer_finished, synced_gpus, device=input_ids.device):
            ...
            # selected_idx, sim_img, sim_context = _ranking_fast(context_hidden, next_hidden, top_k_probs, penalty_alpha, top_k, beta, gamma)
            selected_idx, sim_img, sim_context = _ranking_fast(outputs.encoder_last_hidden_state, next_hidden, top_k_probs, penalty_alpha, top_k, beta, gamma)
...
# 
            ...
            for i in range (0, len(sim_img)):

                if next_tokens[i].item() != 50118:
                    sim_img[i] = 0
                else:
                    sim_img[i] = -0.1
            if len(sequence_sim_img) == 0:
                sequence_sim_img=sim_img.unsqueeze(1)
                sequence_sim_context=None

            else:
                sequence_sim_img = torch.cat((sequence_sim_img,sim_img.unsqueeze(1)), dim=1)
                # sequence_sim_context = torch.cat((sequence_sim_context,sim_context.unsqueeze(1)), dim=1)
        ...
        if return_dict_in_generate:
            ...
            if self.config.is_encoder_decoder:
                return GenerateEncoderDecoderOutput(
                    ...
                    scores=scores,
                    logits=raw_logits,
                    similarities = [sequence_sim_img,sequence_sim_context],
...
def _ranking_fast_old(
    ...
    #reverse the objective for image tokens and see how it works
...
def _ranking_fast(
    ...
    # norm_context_hidden = context_hidden / context_hidden.norm(dim=2, keepdim=True)
    # cosine_matrix = torch.matmul(norm_context_hidden, norm_next_hidden.transpose(1, 2)).squeeze(-1)  # [B*K, S]
    # degeneration_penalty = torch.argmax(cosine_matrix, dim=-1)  # [B*K]
    # degeneration_penalty
    # breakpoint()
    # image_hidden = context_hidden[:,0:32,:]
    image_hidden = context_hidden

    # context_only_hidden = context_hidden[:,32::,:]


    # norm_context_only_hidden = context_only_hidden / context_only_hidden.norm(dim=2, keepdim=True)
    norm_image_hidden = image_hidden / image_hidden.norm(dim=2, keepdim=True)
    norm_next_hidden = next_hidden / next_hidden.norm(dim=2, keepdim=True)
    # cosine_matrix_context = torch.matmul(norm_context_only_hidden, norm_next_hidden.transpose(1, 2)).squeeze(-1)  # [B*K, S]
    cosine_matrix_image = torch.matmul(norm_image_hidden, norm_next_hidden.transpose(1, 2)).squeeze(-1)  # [B*K, S]
    topk_image_sim, _ = torch.topk(cosine_matrix_image, k=3, dim=1, largest=True)
    # degeneration_penalty_context, _ = torch.max(cosine_matrix_context, dim=-1)  # [B*K]
    # degeneration_penalty_context = 1 - degeneration_penalty_context
    # degeneration_penalty_image, _ = torch.max(cosine_matrix_image, dim=-1)  # [B*K]
    degeneration_penalty_image = torch.mean(topk_image_sim, dim=-1)  # [B*K]
    next_top_k_probs = next_top_k_probs.view(-1)  # [B*K]
    #reverse the objective for image tokens and see how it works
    # contrastive_score = (1.0 - alpha) * next_top_k_probs - alpha * degeneration_penalty_image
    contrastive_score =  next_top_k_probs 

    # contrastive_score = (1.0 - alpha) * next_top_k_probs  + 0.7*degeneration_penalty_image - 1*degeneration_penalty_context

    # contrastive_score = (1.0 - alpha) * next_top_k_probs - alpha * degeneration_penalty_context 
    contrastive_score = torch.stack(torch.split(contrastive_score, beam_width))  # [B, K]
    similarities = torch.stack(torch.split(degeneration_penalty_image, beam_width))
    _, selected_idx = contrastive_score.max(dim=-1)  # [B]
    selected_sim = similarities[torch.arange(similarities.size(0)), selected_idx]
    baseline_sim = similarities.mean(dim=1)
    final_sim_img = selected_sim - baseline_sim

    # similarities = torch.stack(torch.split(degeneration_penalty_context, beam_width))
    # selected_sim = similarities[torch.arange(similarities.size(0)), selected_idx]
    # baseline_sim = similarities.mean(dim=1)
    # final_sim_context = selected_sim - baseline_sim
    return selected_idx, final_sim_img, None
