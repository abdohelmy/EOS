    ...
    with tqdm( unit='it', total=len(dataloader_val)) as pbar:
        ...
        for it, (_,_, captions,_,images, prompts, ids,_) in enumerate(dataloader_val):
            ...
            with torch.no_grad():
                ...
                # generated_ids = model.model_blip.generate(input_ids=prompts, pixel_values=images, max_length=60,num_beams= 5)
                generated_ids_contrastive  = model.model_blip.generate(input_ids=prompts, pixel_values=images, max_length=100, output_logits= True, return_dict_in_generate=True) #greedy
                generated_ids_beam = model.model_blip.generate(input_ids=prompts, pixel_values=images, max_length=60,num_beams= 5, no_repeat_ngram_size=2, repetition_penalty=1.5)
...
def recaptioning(things_coco_dataloader_train, coco_dataloader_val, coco_cider_dataloader, ref_caps, model_captioning, model_downstream, optimizer1, optimizer2, weights):
    ...
    loss_fn = torch.nn.CrossEntropyLoss(reduction='none')
    # model_captioning_state_dict = torch.load('/workspace/T5_pod/T5_checkpoint_last.pth', map_location='cpu')
...
#             quit()
            ...
            with tqdm( unit='it', total=len(things_coco_dataloader_train)) as pbar:
                try:
                    ...
                    for  it,(label, answer, captions, ques, images, prompts, image_id,_) in  enumerate(things_coco_dataloader_train):
                        # if it%250==0:
                        ...
                        if it%75==0:
                            ...
                            if it ==0:
                                eos_start = probs_mean
                                lowest_prop = rest_probs_mean
                            ...
                            if probs_mean> eos_start+0.07 and it>300:
                                torch.save(model_captioning.state_dict(), './T5_checkpoint_peak_5e8.pth')
                                flag = 1
                            if flag ==1 and probs_mean < eos_start:
                                torch.save(model_captioning.state_dict(), './T5_checkpoint_stable_5e8.pth')
                            ...
                            if lowest_prob>rest_probs_mean+0.01 and flag==1 and patience !=10:
                                lowest_prob = rest_probs_mean
                                patience=0
                            else:
                                patience+=1
                            if patience == 5:
                                torch.save(model_captioning.state_dict(), './T5_checkpoint_last_5e8.pth')
                                patience = 10
                                # quit()
                    ...
                    # with torch.autograd.set_detect_anomaly(True):
                        ...
                        # try:
                        # if it<50 or it>120:
                        outs  = model_captioning.model_blip.generate(input_ids=prompts, pixel_values=images,penalty_alpha=0.6, top_k=3,  max_length=50, output_logits= True, return_dict_in_generate=True  )
...
################################################################################
                        ...
                        if it%300==0:
                                torch.save(model_captioning.state_dict(), './T5_checkpoint_5e8.pth')
