...
#        self.processor = AutoProcessor.from_pretrained("Salesforce/blip2-flan-t5-xl-coco")
#     #    self.processor2 = AutoProcessor.from_pretrained("Salesforce/blip2-flan-t5-xl-coco")
#        self.hf = h5py.File(self.img_root, 'r')
...
class Coco_Dataset_VQA(Dataset):
    ...
    #    self.processor = AutoProcessor.from_pretrained("Gregor/mblip-mt0-xl")
       self.processor = AutoProcessor.from_pretrained("Salesforce/blip2-flan-t5-xl-coco")
       self.hf = h5py.File(self.img_root, 'r')
    ...
    def __getitem__(self, i):
        ...
        return label,caption, ques, answer, image, img_id, clip_img
...
#pad the labels to be equal to inputs padding
        ...
        prompt_list = [prompt for i in range(0,len(ques))]
        prompt_list = self.processor.tokenizer(text= [example for example in prompt_list], padding='longest', return_tensors="pt")["input_ids"]
        cilp = [example[6] for example in batch]

        return label, answer, caption, ques, images, prompt_list, ids, cilp
...
class FineCapEval(Dataset):
    ...
    #    self.processor = AutoProcessor.from_pretrained("Salesforce/blip2-opt-2.7b")
       self.processor = AutoProcessor.from_pretrained("Salesforce/blip2-flan-t5-xl-coco")
       self.hf = h5py.File(self.img_root, 'r')
       self.images_path = "/l/users/abdelrahman.mohamed/pali/FineCapEval/images"
 

       self.data = pd.read_csv("/l/users/abdelrahman.mohamed/pali/FineCapEval/FineCapEval.csv")
    #    breakpoint()
