...
things_dataloader_train = DataLoader(coco_dataset, batch_size=12,shuffle=True, num_workers=1,collate_fn = coco_dataset.collate_fn,
...
def train_captioning(model, dataloader_train, optimizer, dataloader_val, ref_caps):
    model = model.to(device)
    model.train()
    ref_caps = load_references_from_json_val("./my_datasets/captions_val2014.json")
    running_loss = 0
    for name, param in model.model_blip.named_parameters():
        param.requires_grad = False
    for name, param in model.model_blip.qformer.named_parameters():
        param.requires_grad = True
    for name, param in model.model_blip.language_projection.named_parameters():
        param.requires_grad = True
    with wandb.init(project="recap", mode=WANDB_STATUS, notes=NOTES, name=EXP_NAME):
            ...
            with tqdm(desc='Epoch %d - train' % e, unit='it', total=len(dataloader_train)) as pbar:
                for it, (images, captions, mask, prompts, ids) in enumerate(dataloader_train):
                    if it%75==0:
                        ...
                        captions_beam, probs_mean, rest_probs_mean, avg_length, recalls= val_evaluation(model, dataloader_val, ref_caps)
                    ...
                    try:
                        ...
                        outputs = model(input_ids=prompts ,pixel_values=images, labels=captions)
                        ...
                        pbar.update()
                                    ...
                                    print(f)
                    if it%300==0:
                                torch.save(model.state_dict(), './SMILE.pth')
...
ref_caps = []
def evaluation(model, dataloader_val, ref_caps, rep):
    # ref_caps = load_references_from_json_finecap()
    # ref_caps = load_references_from_json_dci()
    ref_caps = load_references_from_json("./my_datasets/annotations/captions_val2014.json")
    ...
    with tqdm( unit='it', total=len(dataloader_val)) as pbar:
        ...
        for it, (_,_, captions,_,images, prompts, ids) in enumerate(dataloader_val):
            ...
            with torch.no_grad():
                ...
                generated_ids_beam = model.model_blip.generate(input_ids=prompts, pixel_values=images, max_length=100,num_beams= 5, no_repeat_ngram_size=2, repetition_penalty=rep)
    ...
    return None, score_beam_c, None, score_beam_r, None, recall_b.mean(), None, CHAIR_data_beam
...
def val_evaluation(model, dataloader_val, ref_caps):
    ...
    with tqdm( unit='it', total=len(dataloader_val)) as pbar:
        ...
        for it, (_,_, captions,_,images, prompts, ids) in enumerate(dataloader_val):
            ...
            with torch.no_grad():
                ...
                # generated_ids = model.model_blip.generate(input_ids=prompts, pixel_values=images, max_length=60,num_beams= 5)
                generated_ids_contrastive  = model.model_blip.generate(input_ids=prompts, pixel_values=images, max_length=60, output_logits= True, return_dict_in_generate=True, no_repeat_ngram_size=2)
    ...
    return gen_caps_contrastive, np.mean(all_probs), np.mean(all_probs_rest), avg_len/it, recall_contrastive.mean()
...
# def evaluate_capture(ref_captions, gen_captions):
#     scorer = CAPTURE()
#     capture_score, _ = scorer.compute_score(gen_captions, ref_captions)
...
#                     del model_captioning_state_dict
            trained_models = [
                              ...
                              "T5_best.pth",
                            #   "recaptioning_T5_last_eos_035.pth"


                            
]       
            repetiton_val = [1,1.1,1.2,1.3,1.4,1.6]
            for mod in trained_models:
                    ...
                    # del model_captioning_state_dict
                    print(mod)
                    for repet in repetiton_val:
                        c_contrastive, c_beam, r_contrastive, r_beam, recall_contrastive, recall_beam, dict_base, dict_base_beam = evaluation(model_captioning, coco_dataloader_val, ref_caps, repet)

                        # with open("dict_T5_last_.jsonl", "w") as f:
                        #     for item in dict_base:
                        #         f.write(json.dumps(item) + "\n")
                        with open("eval_T5_"+str(repet)+"penalty_"+str(mod)+"coco.jsonl", "w") as f:
                            for item in dict_base_beam:
                                f.write(json.dumps(item) + "\n")
                        with open("chair.jsonl", "w") as f:
                            for item in dict_base_beam:
                                    f.write(json.dumps(item) + "\n")
                        valuator = pickle.load(open("/l/users/yova.kementchedjhiev/abdo/recap/chair.pkl", 'rb'))
                        chair_dic = valuator.compute_chair("./chair.jsonl","image_id","caption")
                        chair_i = chair_dic["overall_metrics"]['CHAIRi']*100
                        chair_s = chair_dic["overall_metrics"]['CHAIRs']*100
                        print("############CHAIR_I######################"+ str(chair_i))
                        print("############CHAIR_S######################"+ str(chair_s))
                        print("##########CIDER_contrastive########: "+ str(c_contrastive))
                        print("##########CIDER_beam########: "+ str(c_beam))
                        print("##########rouge_contrastive########: "+ str(r_contrastive))
                        print("##########rouge_beam########: "+ str(r_beam))
            ...
            with tqdm( unit='it', total=len(things_coco_dataloader_train)) as pbar:
                ...
                for  it,(label, answer, captions, ques, images, prompts, image_id) in  enumerate(things_coco_dataloader_train):
                        ...
                        #     gc.collect()
                        if it%50==0:
                            captions, probs_mean, rest_probs_mean, avg_length= val_evaluation(model_captioning, coco_dataloader_val, ref_caps)
                            model.train()
                            wandb.log({"eos_val_prob":probs_mean})
                            wandb.log({"rest_val_prob":rest_probs_mean})
                            wandb.log({"val_length":avg_length})
                    ...
                    # with torch.autograd.set_detect_anomaly(True):
                        ...
                        with torch.no_grad():
                            ## Greedy decode captioning model
                            generated_tokens_greedy = model_captioning.model_blip.generate(input_ids=prompts, pixel_values=images, max_length=40)
                            generated_caption_baseline = processor.batch_decode(generated_tokens_greedy, skip_special_tokens=True)
...
################################################################################
                        ...
                        #     continue
                        # breakpoint()
                        loss = -torch.mean(words_prob * reward, -1)
...
if __name__ == "__main__":
    ...
    train_captioning(model_blip, things_dataloader_train, optimizer1, vqa_val_dataloader_rec, ref_caps )
...
# ref_caps = load_references_from_json("./annotations/captions_val2014.json")
