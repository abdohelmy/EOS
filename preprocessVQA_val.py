...
def preprocess_json(input_file, output_records_file, output_mapping_file):
    ...
    with open("/home/israfel.salazar/abdo/Recaption/my_datasets/v2_mscoco_train2014_annotations.json", 'r') as f:
    ...
    for record in records_train["annotations"]:
        for ans in record['answers']:
            if ans['answer_confidence'] != 'no':  # Only consider answers with confidence other than 'no'
                ...
                if len(answer_lower.split()) <= 2:
                    if record["question_type"] in ques_type:
                        all_answers_train.append(answer_lower)
    ...
    processed_records = []
    with open("/home/israfel.salazar/abdo/Recaption/my_datasets/v2_OpenEnded_mscoco_val2014_questions.json") as f:
...
preprocess_json('/home/israfel.salazar/abdo/Recaption/my_datasets/v2_mscoco_val2014_annotations.json', 'what_1000_multi_val.json', 'what_1000_multi_mapping_val.json')
