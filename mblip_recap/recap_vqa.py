...
def recaptioning(things_coco_dataloader_train, coco_dataloader_val, coco_cider_dataloader, ref_caps, model_captioning, model_downstream, optimizer1, optimizer2, weights):
    ...
    loss_fn = torch.nn.CrossEntropyLoss(reduction='none')
    model_captioning_state_dict = torch.load('/workspace/T5_pod/T5_checkpoint2.pth', map_location='cpu')
                ...
                try:
                    ...
                    for  it,(label, answer, captions, ques, images, prompts, image_id) in  enumerate(things_coco_dataloader_train):
                        ...
                        if it%75==0:
                            ...
                            #     patience+=1
                            # if patience == 3:
                            #     torch.save(model_captioning.state_dict(), './mblip_checkpoint_last.pth')
                            #     quit()
                    ...
                    # with torch.autograd.set_detect_anomaly(True):
                        ...
                        # try:
                        # if it<50 or it>120:
                        outs  = model_captioning.model_blip.generate(input_ids=prompts, pixel_values=images,penalty_alpha=0.6, top_k=2,  max_length=60, output_logits= True, return_dict_in_generate=True  )
...
################################################################################
                        ...
                        if it%300==0:
                                torch.save(model_captioning.state_dict(), './mblip_checkpoint.pth')
