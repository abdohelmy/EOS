...
class bert_recaption(nn.Module):
    ...
    def forward(self, tokens, attn_mask):
        ...
        x = self.model(input_ids = tokens, attention_mask= attn_mask)["last_hidden_state"]
        output = self.vqa(x)
...
class blip_recaption(Blip2ForConditionalGeneration):
    def __init__(self): 
        ...
        self.model_blip = Blip2ForConditionalGeneration.from_pretrained("Gregor/mblip-mt0-xl", cache_dir = "./mblip",)
        # self.model_blip = mBLIP.from_pretrained("Gregor/mblip-mt0-xl", cache_dir = "./Mblip2_coco_T5")
